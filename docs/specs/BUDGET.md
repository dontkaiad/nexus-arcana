...
- Железные минимумы (продукты/транспорт/импульсивные) в промпте Sonnet
  суммируются в **15 500₽** (после того как бьюти убрали из минимумов),
  но два UI-порога «показать ⚠️ жёстко» в `_run_budget_analysis` и
  `_format_plan` (`nexus/handlers/finance.py`, буквально `18500` в коде)
  всё ещё сравнивают со **старым числом 18 500₽**. Не ломает расчёт
  лимитов, но плашка «жёстко» может появляться не в той точке, где
  реально начинаются железные минимумы.

...
### 6. Бот — отдельный `@heylark_booking_bot` (пересмотр)
...
приватной поверхности Nexus. Решение: **отдельный бот
`@heylark_booking_bot`.** Не Nexus, не Arcana, не `l4rk_sys_bot` (тот
send-only, только логи).
...
Отдельный небольшой Vite-app (не 9600-строчный `App.jsx`), Caddy vhost
`booking.heylark.dev`. Дизайн glass-cards, общие CSS-токены, тема по контексту.

- **guest** — сетка занятости без деталей, без брони.
- **friend** (`hl_session` + `grants`) — реальные слоты, флоу «как в planerka»
  (тип встречи → окна → слот → форма/подтверждение, автоконверсия TZ).
- **admin** (Кай) — **полное управление календарём прямо на сабдомене**: сетка
  с содержимым, входящие заявки (подтвердить/отклонить/перенести), редактор
  окон доступности, типы встреч, блоки, история броней. Отдельной вкладки в
  Mini App не делаем — управление живёт здесь.

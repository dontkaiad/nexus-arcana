...
| окна доступности | свои (`booking_availability`, весь §7 ниже) — **был короткий пересмотр (#233): без окон, любой свободный час 13:00–23:00 МСК; откачен обратно (#243)** — Кай не понравился всегда-открытый календарь | свои (`booking_availability`, весь §7 ниже) |
...
- **`friends`: окна снова обязательны** (§3 — #233 введён и тем же сезоном
  откачен #243) — один windows-based алгоритм на оба контекста
  (`core/booking/slots.py:free_slots`), opt-in: без `booking_availability`
  строки на день — слотов ноль ✅
...
- **Повод обязателен** (#239) — `note` = характер встречи ("шашлыки в
  парке"), не имя заявителя; имя уходит в заметку созданной задачи ✅

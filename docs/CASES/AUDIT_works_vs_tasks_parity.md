...
| Стрики (per-task / global) + rest-day | Global: `nexus/handlers/streaks.py` (`update_streak`, `get_streak`, `request_rest_day`, `is_rest_day_available`). Per-task: `core/task_streaks.py` (`update_task_streak`, `reset_broken_streaks`), вызывается из `tasks.py:1415-1436` (recurring done) и `:2217-2227` (`_update_streak_line`, обычное done) | — | **WON'T DO (design decision) — см. ADR-0023 (`docs/CASES/0023-streaks-out-of-scope-for-arcana-works.md`).** `core/task_streaks.py` technически generic (per `user_id, task_id`, без FK), но его continuation-логика построена на `repeat_kind`, а вызывается он в Nexus только для ПОВТОРЯЮЩИХСЯ задач. Работы Арканы — одноразовые клиентские записи, не привычки; форсировать стрик через фиктивный `repeat_kind` дало бы мусорную метрику. Кай подтвердила: стрики Аркане не нужны. Не бэклог-пункт, а закрытое решение |
| Repeat + repeat_time | Схема: `nexus/repos/tasks_tables.py:47,54` (`repeat_id` FK, `repeat_time` Text). Логика: `_parse_repeat_time` (`tasks.py:1186`), `_next_cycle_date` (`:1217`), recurring reset/reminder/deadline done (`:1279-1445`), oживление на старте (`restore_reminders_on_startup` проходы 2-3, `:286-469`) | Схема (миграция `ab12cd34ef56`): `work_repeat`/`work_day_of_week` lookups + `works.repeat_id`/`day_of_week_id`/`repeat_time` — те же имена/типы что у tasks. Логика: общий `core/recurrence.py` (`parse_repeat_time`/`next_cycle_date`/`interval_label` — вынесены из `tasks.py`, оба бота импортируют), `_handle_recurring_work_done` (`arcana/handlers/work_reminder_kb.py`). Парсинг из текста — `PARSE_WORK_SYSTEM` (Haiku, как у Nexus). БЕЗ стриков (ADR-0023). | **✅ DONE.** Повторяющиеся Работы: repeat парсится из текста, завершение сдвигает дедлайн/напоминание на следующий цикл (статус остаётся `open`, у Работ нет `In progress`), напоминание перепланируется через общий `core/reminder_scheduler.py`. Не портировано (follow-up): «оживление на старте» повторяющихся Работ с `repeat_time` но `reminder IS NULL` (проход 3 Nexus), Mini-App-фильтр «выполнено сегодня» |

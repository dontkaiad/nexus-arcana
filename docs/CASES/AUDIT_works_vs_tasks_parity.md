...
| Repeat + repeat_time | Схема: `nexus/repos/tasks_tables.py:47,54` (`repeat_id` FK, `repeat_time` Text). Логика: `_parse_repeat_time` (`tasks.py:1186`), `_next_cycle_date` (`:1217`), recurring reset/reminder/deadline done (`:1279-1445`), oживление на старте (`restore_reminders_on_startup` проходы 2-3, `:286-469`) | — | **Отсутствует на уровне схемы. Открытый пункт — будет портирован отдельно, со своим ADR.** `arcana/repos/works_tables.py:34-51` — таблица `works` НЕ содержит `repeat_id`/`repeat_time` вообще. Это не «функция не подключена», это «колонки физически нет» — портирование требует миграции Alembic, не только handler-кода. **Нужен отдельно от стриков** (repeat = recurrence, стрики = habit-метрика; см. ADR-0023, раздел «Out of scope — repeat») — портирование repeat НЕ тянет за собой стрики |

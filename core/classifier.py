...
def _next_abs_date_iso(day: int, month: int) -> str:
    """Ближайшая дата ДД.ММ (для примеров в промпте) — этот год, либо следующий если уже прошла."""
    today = datetime.now().date()
    year = today.year
    try:
...
def build_system(tz_offset: int = 3) -> str:
    ...
    return "\n".join([
        ...
        '{"type":"edit_record","record_type":"task|finance","record_hint":"поисковые слова","field":"category|priority|title|deadline|status|duration|shared|note","new_value":"новое значение"}',
        ...
        "         'добавь к задаче сдать биометрию заметку: адрес такой-то' → edit_record field=note new_value='адрес такой-то' (new_value — весь текст заметки как есть, не сокращай)",
        ...
        "         'в сбер я сходила' → task_done hint='сбербанк'",
        "         'съездила на почту' → task_done hint='почта'",
        "         'заехала в аптеку' → task_done hint='аптека'",
        "ВАЖНО: глаголы движения в прошедшем времени (сходила/съездила/заехала/зашла) про поручение = task_done, порядок слов любой ('в сбер я сходила' = 'сходила в сбер')",
        "ВАЖНО: task_done только если речь идёт о чём-то уже сделанном (прошедшее время глагола или слово 'готово'/'выполнено')",
        "",
        "task_cancel — пользователь ОТМЕНЯЕТ задачу/событие (не сделала, а убирает/отменила):",
        '{"type":"task_cancel","task_hint":"ключевые слова названия задачи"}',
        "Примеры: 'отменила встречу с врачом' → task_cancel hint='встреча врач'",
        "         'отмени задачу позвонить маме' → task_cancel hint='позвонить маме'",
        "         'сними йогу на сегодня' → task_cancel hint='йога'",
        "         'не пойду к стоматологу, отменяю' → task_cancel hint='стоматолог'",
        "ВАЖНО: 'отменил/отменила/отменили/сними/убери X' = task_cancel (задачу убирают, она НЕ выполнена), НЕ task_done!",
        "",
        "task — задача на Нексус (БЕЗ слов из Арканы!):",
        '{"type":"task","title":"что сделать","category":"<из кат>","priority":"Срочно|Важно|Можно потом","deadline":"YYYY-MM-DDTHH:MM или null","reminder":"YYYY-MM-DDTHH:MM или null","repeat":"Нет|Ежедневно|Еженедельно|Ежемесячно","repeat_time":"HH:MM или null","day_of_week":"Пн|Вт|Ср|Чт|Пт|Сб|Вс или null","duration":"сырое упоминание длительности как в тексте (напр. «2 часа», «30 минут») или null","note":"сырые денежные/прочие детали задачи (напр. «оплата 1250 рублей») или null","confidence":"high если есть дата или repeat, low иначе"}',
        "КРИТИЧНО про duration: если явно указана длительность встречи/дела («на 2 часа», «длительность 2 часа», «на полчаса») — положи СЫРОЙ текст длительности в поле duration (не считай минуты сама, это сделает отдельный парсер). Без явного упоминания длительности — duration=null.",
        "  'встреча с Х завтра в 17:30 длительность 2 часа' → duration=\"2 часа\"",
        "  'созвон в 15:00 на полчаса' → duration=\"полчаса\"",
        "  'купить корм коту' → duration=null",
        "КРИТИЧНО про note: если сообщение описывает ЗАДАЧУ с будущим дедлайном/временем И упоминает сумму денег как ЧАСТЬ этой будущей задачи (оплата произойдёт в день дедлайна, а не сейчас) — НЕ создавай отдельный item \"expense\". Положи сырое упоминание в note (напр. note=\"оплата 1250 рублей\"), а title — без суммы. Отдельный expense — ТОЛЬКО про уже случившуюся трату (прошедшее время: «заплатила», «потратила», «стоило»).",
        "  'прийти к нотариусу в среду, оплата 1250' → ОДИН item: {\"type\":\"task\",\"title\":\"прийти к нотариусу\",...,\"deadline\":\"<ближайшая среда>\",\"note\":\"оплата 1250 рублей\"} — БЕЗ expense.",
        "  'кофе 180 рублей' (нет признаков будущей задачи) → {\"type\":\"expense\",\"amount\":180,...} как обычно.",
        "  'заплатила 1250 нотариусу' (прошедшее время, уже произошло) → {\"type\":\"expense\",\"amount\":1250,...}, НЕ note.",
        "КРИТИЧНО про слово «заметка» ПРИ СОЗДАНИИ задачи: если сообщение целиком про ОДНО дело (есть title задачи) и слово «заметка»/«заметку» вводит доп. детали ИМЕННО К ЭТОМУ делу (как/где/когда его делать, что взять с собой, порядок действий) — это ОДИН item type=task, детали после «заметка» кладутся в поле note дословно. НЕ создавай отдельный item type=note — иначе получится задача И отдельная заметка из одного сообщения (двойное действие).",
        "  'задача получить загран, как будет готов заметка подойти в окно 5 по очереди, не надо заранее записываться' → ОДИН item: {\"type\":\"task\",\"title\":\"получить загран\",...,\"note\":\"как будет готов — подойти в окно 5 по очереди, не надо заранее записываться\"} — БЕЗ отдельного note.",
        "Отдельный item type=note — ТОЛЬКО если сообщение НЕ описывает конкретное дело/задачу, а целиком самостоятельная идея/мысль (см. note ниже).",
        "КРИТИЧНО: фраза 'посчитать/узнать/выяснить сколько X должен/задолжал денег' — это ЗАДАЧА (посчитать долг), НЕ expense/income/they_owe (там нет суммы и не прошедшее время). Слово 'дедлайн' в любом месте текста — явный маркер deadline, даже без слов 'сегодня/до пятницы': парсить дату/время после него по общим правилам deadline (см. ниже).",
        "  'посчитать сколько миша мне должен денег дедлайн завтра в 13' → {\"type\":\"task\",\"title\":\"посчитать сколько миша должен денег\",\"priority\":\"Срочно\",\"deadline\":\"<завтра>T13:00\",...}",
        ...
        "- ЗАДАЧА С БУДУЩЕЙ ОПЛАТОЙ: 'записаться к нотариусу на четверг, пошлина 2000' / 'прийти в среду, оплата 1250' → task + note='пошлина 2000 рублей', БЕЗ отдельного expense (деньги уйдут в день дедлайна, не сейчас).",
        ...
        "- ЦИФРОВАЯ ДАТА В СЕРЕДИНЕ/КОНЦЕ ТЕКСТА (формат ДД.ММ или ДД.ММ.ГГГГ, с временем или без): ВСЕГДА извлекай в deadline (или reminder — по общим правилам выше), title — только остальной текст БЕЗ даты и времени. Это работает даже если title — не глагол, а название/место/существительное (такси, встреча, поход, бронь и т.д.), и даже если между названием и датой стоит тире. Дата НЕ является частью названия. Год не указан → текущий год " + today[:4] + ", если дата уже прошла в этом году → следующий.",
        "  Пример: 'такси Стерлитамак — Гай 20.08 в 16:00' → title='такси Стерлитамак — Гай' (БЕЗ '20.08 в 16:00'), deadline='<год>-08-20T16:00'",
        "  Пример: 'встреча с нотариусом 28.08 10:00' → title='встреча с нотариусом' (БЕЗ '28.08 10:00'), deadline='<год>-08-28T10:00'",
        ...
        "- КРИТИЧНО «напомни»+ВРЕМЯ → REMINDER (не deadline!): если есть слово 'напомни/напомнить/напоминание' И указано время → reminder=YYYY-MM-DDTHH:MM, deadline=null. БЕЗ слова 'напомни' одно время → это deadline (reminder=null). Два времени ('встреча в 15 напомни в 13:40') → deadline=15:00 И reminder=13:40.",
        "-   'напомни в воскресенье в 19' → ОДНОРАЗОВАЯ: repeat='Нет', reminder=ближайшее вс С ВРЕМЕНЕМ (YYYY-MM-DDT19:00), deadline=null",
        "-   'напомни в субботу в 11 утра' → ОДНОРАЗОВАЯ: repeat='Нет', reminder=ближайшая сб С ВРЕМЕНЕМ (YYYY-MM-DDT11:00), deadline=null",
        "-   'напомни в пятницу' (БЕЗ времени) → ОДНОРАЗОВАЯ: repeat='Нет', deadline=ближайшая пт (БЕЗ времени, YYYY-MM-DD), reminder=null",
        ...
        "- unknown — ТОЛЬКО если текст вообще не описывает никакое действие/факт/трату (мусор, случайные символы, приветствие без содержания). Если текст — глагол/действие в инфинитиве или повелительном наклонении ('посчитать', 'узнать', 'сделать X'), пусть даже с деньгами/долгами без явной суммы — это task с confidence=low, НЕ unknown.",
        "",
        ...
        '  Ответ: {"type":"task","title":"позвонить врачу","category":"🏥 Здоровье","priority":"Важно","deadline":"' + _next_weekday_iso("Пт") + '","reminder":null,"repeat":"Нет","repeat_time":null,"day_of_week":null,"confidence":"high"}',
        "  Ввод: 'позвонить маме в 18' (БЕЗ 'напомни', одно время → DEADLINE, reminder=null)",
        '  Ответ: {"type":"task","title":"позвонить маме","category":"💳 Прочее","priority":"Важно","deadline":"' + today + 'T18:00","reminder":null,"repeat":"Нет","repeat_time":null,"day_of_week":null,"confidence":"high"}',
        "  Ввод: 'встреча в 15 напомни в 13:40' (два времени → DEADLINE 15:00 + REMINDER 13:40)",
        '  Ответ: {"type":"task","title":"встреча","category":"💳 Прочее","priority":"Важно","deadline":"' + today + 'T15:00","reminder":"' + today + 'T13:40","repeat":"Нет","repeat_time":null,"day_of_week":null,"confidence":"high"}',
        "",
        "ПРИМЕР ЦИФРОВАЯ ДАТА В ЗАГОЛОВКЕ-СУЩЕСТВИТЕЛЬНОМ (дата НЕ уходит в title!):",
        "  Ввод: 'такси Стерлитамак — Гай 20.08 в 16:00' (title — не глагол, дата ДД.ММ приклеена в конце → всё равно DEADLINE, НЕ часть title)",
        '  Ответ: {"type":"task","title":"такси Стерлитамак — Гай","category":"🚕 Транспорт","priority":"🟡 Важно","deadline":"' + _next_abs_date_iso(20, 8) + 'T16:00","reminder":null,"repeat":"Нет","repeat_time":null,"day_of_week":null,"confidence":"high"}',
        "  Ввод: 'поход к нотариусу 28.08 10:00 напомни 27.08 в 12' (две даты ДД.ММ, вторая с 'напомни' → DEADLINE 28.08 10:00 + REMINDER 27.08 12:00, обе даты вне title)",
        '  Ответ: {"type":"task","title":"поход к нотариусу","category":"💳 Прочее","priority":"🟡 Важно","deadline":"' + _next_abs_date_iso(28, 8) + 'T10:00","reminder":"' + _next_abs_date_iso(27, 8) + 'T12:00","repeat":"Нет","repeat_time":null,"day_of_week":null,"confidence":"high"}',
        "",
...
# issue #158: ловим прошедшее время («отменила встречу») + объект — это про
# отменённую задачу/событие. handle_task_cancel сам fuzzy-найдёт задачу (или
# ответит graceful «не нашёл»). Проверяется ДО _DONE_RE, чтобы «отменила X» не
# уехало в task_done. Императив («отмени») оставляем привязанным к слову
# «задача», иначе «отмени заметку» (операция с заметкой) ушло бы в task_cancel.
_CANCEL_RE = re.compile(
    r"\b(отменил[аи]?)\b\s+\S+"
    r"|\b(отмени|отмените|отменить|отмена)\b.{0,50}\b(задач\w*)\b"
...
_DONE_RE = re.compile(
    ...
    # глаголы движения/действия прошедшего времени — «в сбер я сходила»,
    # «съездила на почту», «заехала в аптеку», «сдала отчёт», «оплатила счёт».
    # currency-guard на строке вызова защищает от «оплатила счёт 3000».
    r"сходил[аи]\b|съездил[аи]\b|заехал[аи]\b|зашл[аи]\b|"
    r"сгоняла\b|смоталась\b|сдал[аи]\b|оплатил[аи]\b|"
...
_DEBT_CMD_RE = re.compile(
    r"(?:закрыла?\s+долг|новый\s+долг|отдала?\s.*долг|отдала?\s+\S+\s+\d|погасила?)",
    re.IGNORECASE,
)
...
_EDIT_PARSE_SYSTEM = (
    ...
    "'добавь к задаче сдать биометрию заметку: адрес такой-то, с собой паспорт' → record_hint='сдать биометрию', edits=[{\"field\":\"note\",\"new_value\":\"адрес такой-то, с собой паспорт\"}]\n"
)

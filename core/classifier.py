...
def build_system(tz_offset: int = 3) -> str:
    ...
    return "\n".join([
        ...
        "",
        'Вход: "погладить кота каждый день в 9" → {"type":"task","title":"погладить кота","category":"🐾 Коты","priority":null,"deadline":null,"repeat":"Ежедневно","repeat_time":"09:00","day_of_week":null,"confidence":"high"}',
        'Вход: "йога каждую пятницу в 18" → {"type":"task","title":"йога","category":"📚 Хобби/Учеба","priority":null,"deadline":null,"repeat":"Еженедельно","repeat_time":"18:00","day_of_week":"Пт","confidence":"high"}',
        'Вход: "написать Маше" → {"type":"task","title":"написать Маше","category":"👥 Люди","priority":null,"deadline":null,"repeat":"Нет","repeat_time":null,"day_of_week":null,"confidence":"high"}',
        ...
        '{"type":"task","title":"что сделать","category":"<из кат>","priority":"Срочно|Важно|Можно потом|null","deadline":"YYYY-MM-DDTHH:MM или null","repeat":"Нет|Ежедневно|Еженедельно|Ежемесячно","repeat_time":"HH:MM или null","day_of_week":"Пн|Вт|Ср|Чт|Пт|Сб|Вс или null","confidence":"high если есть дата или repeat, low иначе"}',
        "Приоритет задачи — ставь ТОЛЬКО если есть явные маркеры:",
        "  Срочно: слова 'срочно','немедленно','сейчас','asap', дедлайн сегодня",
        "  Важно: здоровье, деньги, работа, дедлайн скоро (1-3 дня)",
        "  Можно потом: 'когда-нибудь','как-нибудь','при случае', нет дедлайна + мелочь",
        "  null: нет явных маркеров приоритета → НЕ угадывай, ставь null",
        "",
        ...
        '  Ответ: {"type":"task","title":"платить за аренду","category":"🏠 Ж***","priority":"Средний","deadline":null,"repeat":"Ежемесячно","repeat_time":null,"day_of_week":null,"confidence":"low"}',
        "",
...
_EDIT_NOTE_RE = re.compile(
    ...
    r"\s+(тег|теги|метку|метки|категорию)(\s+заметки|\s+последней)?",
    re.IGNORECASE,
)

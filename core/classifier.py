...
from core.list_classifier import (
    ...
    _looks_like_med_inventory, looks_like_buy_continuation,
    LIST_HAIKU_TYPES,
)
...
def build_system(tz_offset: int = 3) -> str:
    ...
    return "\n".join([
        ...
        '{"type":"task","title":"что сделать","category":"<из кат>","priority":"Срочно|Важно|Можно потом","deadline":"YYYY-MM-DDTHH:MM или null","reminder":"YYYY-MM-DDTHH:MM или null","repeat":"Нет|Ежедневно|Еженедельно|Ежемесячно","repeat_time":"HH:MM или null","day_of_week":"Пн|Вт|Ср|Чт|Пт|Сб|Вс или null","confidence":"high если есть дата или repeat, low иначе"}',
        "КРИТИЧНО про reminder: ВСЕГДА null, ЕСЛИ в исходном тексте НЕТ слов «напомни / напомнить / напоминалку / напоминай / напоминание / remind». НЕ додумывай напоминание заранее, НЕ ставь «напомню в день дедлайна», НЕ ставь дефолтное время. Reminder ставится ТОЛЬКО когда юзер явно попросил.",
        "Negative examples (reminder = null):",
        "  'поставь задачу через неделю выполнить X' → reminder=null (нет слова напомни)",
        "  'купить корм завтра' → reminder=null",
        "  'сдать отчёт до пятницы' → reminder=null",
        "  'позвонить маме в 18:00' → reminder=null (это deadline с временем, не reminder)",
        "Positive examples (reminder задан):",
        "  'напомни завтра в 15 купить корм' → reminder=завтра T15:00, deadline=null",
        "  'напоминалку на пятницу 10:00 сдать отчёт' → reminder=ближайшая пт T10:00",
        "  'купить корм, напомни в 18' → reminder=сегодня/завтра T18:00",
        ...
        "-   'напомни в воскресенье в 19' → ОДНОРАЗОВАЯ: repeat='Нет', deadline=ближайшее вс С ВРЕМЕНЕМ (YYYY-MM-DDT19:00), reminder_time не ставить",
        "-   'напомни в субботу в 11 утра' → ОДНОРАЗОВАЯ: repeat='Нет', deadline=ближайшая сб С ВРЕМЕНЕМ (YYYY-MM-DDT11:00), reminder_time не ставить",
        "-   'напомни в пятницу' → ОДНОРАЗОВАЯ: repeat='Нет', deadline=ближайшая пт (БЕЗ времени, YYYY-MM-DD)",
        ...
        "- СЛОЖНЫЙ ЧЕК ('из них'): если в тексте паттерн 'СУММА1 МЕСТО из них СУММА2 ТОВАР' → разбить на 2 expense:",
        "  1) amount=СУММА2, title=ТОВАР, category по ТОВАРУ",
        "  2) amount=СУММА1-СУММА2 (остаток), title=МЕСТО, category по МЕСТУ",
        "  Магазины-подсказки: красное белое/кб/перекресток/пятерочка/магнит/лента/ашан/дикси → 🍜 Продукты",
        "  Пример: '1720р красное белое из них 1425 сигареты' →",
        '  [{"type":"expense","amount":1425,"title":"сигареты","category":"🚬 Привычки","source":"💳 Карта","confidence":"high"},',
        '   {"type":"expense","amount":295,"title":"красное белое","category":"🍜 Продукты","source":"💳 Карта","confidence":"high"}]',
        "  Пример: '5к пятерочка из них 2к алкоголь' →",
        '  [{"type":"expense","amount":2000,"title":"алкоголь","category":"🚬 Привычки","source":"💳 Карта","confidence":"high"},',
        '   {"type":"expense","amount":3000,"title":"пятерочка","category":"🍜 Продукты","source":"💳 Карта","confidence":"high"}]',
...
async def classify(text: str, tz_offset: int = 3) -> list[dict]:
    ...
    # "монстры ещё" / "ещё кофе" → list_buy (продолжение списка без глагола).
    # Guards внутри looks_like_buy_continuation: вопросы, суммы, глаголы задач.
    if looks_like_buy_continuation(text):
        logger.info("classify: list_buy continuation («ещё») matched")
        return [{"type": "list_buy", "text": text}]

...
Если не уверен, пиши вопрос в clarification_question."""
    ...
    raw = await ask_claude(text, system=sys, model="claude-haiku-4-5-20251001", temperature=0)
...
async def parse_time_only(text: str, uid: int) -> str:
    ...
    raw = await ask_claude(text, system=sys, max_tokens=50, model="claude-haiku-4-5-20251001", temperature=0)

...
def _assign_waite_slots(
    ...
    без SequenceMatcher):
      ...
      2. ДЫРКА — неподтверждённые слоты восстанавливаются ТОЛЬКО из «грязных»
         (содержащих мисхёрд-слово) спанов дырки, ПОЗИЦИОННО по порядку. Почему
         «грязь», а не похожесть/позиция: спелл-подмена бьёт лишь по мисхёрдам
         (канон-78 защищён whitelist'ом спелла) → исток подмены ВСЕГДА грязный, а
         чистая карта нарратива («король кубков», «десять монет») — нет, даже если
         шарит масть/ранг с фразой слота. Грязные спаны = именно подменённые карты
         дырки, и идут В ТОМ ЖЕ ПОРЯДКЕ, что слоты (диктовка линейна) → zip 1:1.
         (adversarial: позиция не отличала фантом от истока, surface-похожесть —
         тоже (одноимённый двор); «грязь» отличает структурно.)

    None для слота = нет грязного истока в дырке → выше берём enP или Haiku.
    """
    ...
    # 2. дырки: грязные (мисхёрд) спаны → неподтверждённым слотам позиционно 1:1
    prev_sp, prev_rp = -1, -1
    for a_sp, a_rp in anchors + [(n_slots, n_raw)]:
        gap_slots = list(range(prev_sp + 1, a_sp))
        dirty_cands = [
            c for c in range(prev_rp + 1, a_rp) if _span_is_dirty(raw_pairs[c][1])
        ]
        for s, c in zip(gap_slots, dirty_cands):
            out[s] = raw_pairs[c][0]                 # подменённая карта из истока
        # слоты сверх числа грязных истоков → None (вернут enP / Haiku)
        if a_sp < n_slots:

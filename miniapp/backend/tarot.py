...
NUM_TO_WORD: dict[str, str] = {
    ...
    # Количественные из речи (голос/режим A) → порядковое, как в реестре/ключах
    # справочника («шесть кубков» → «шестёрка кубков» → ключ «6 Кубков»).
    "два":   "двойка",   "три":    "тройка",   "четыре": "четвёрка",
    "пять":  "пятёрка",  "шесть":  "шестёрка", "семь":   "семёрка",
    "восемь": "восьмёрка", "девять": "девятка", "десять": "десятка",
    # Фонетические мисхёрды Whisper для двора: «королева» стабильно искажается.
    "крыльева": "королева", "кралева": "королева", "коралева": "королева",
    "крыльево": "королева", "королево": "королева", "каралева": "королева",
}
...
SUIT_FULL: dict[str, str] = {
    ...
    "мячей":     "мечей",     "мячи": "мечей",
    # Именительная масть из речи (голос/режим A) → родительный канон ключа
    # справочника. Без этого «туз пентакли» не матчился к «Туз Пентаклей».
    "пентакли": "пентаклей", "кубки": "кубков", "мечи": "мечей", "жезлы": "жезлов",
...
# Ранг: цифра | англ. слово | русский двор/туз | русское порядковое | русское
# количественное. Русские нужны, чтобы _NUM_RE распознал имя из речи и применил
# SUIT_FULL к масти (иначе масть «пентакли» оставалась бы именительной).
_RANK_WORDS = (
    "ace|page|knight|queen|king|two|three|four|five|six|seven|eight|nine|ten"
    "|туз|паж|рыцарь|королева|король"

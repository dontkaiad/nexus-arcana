...
const MOCK = {
  ...
  finance: {
    ...
    debts: [
      { n: "***", total: 50000, left: 25000, by: "апрель", note: "рассрочка: 25к + 25к в мае" },
    ],
  },
  tasks: [
...
function ArDay({ s, openClient }) {
  const [done, setDone] = useState({});
  const a = MOCK.arcanaDay;
  const moon = moonPhase(new Date(2026, 3, 22)); // 22 апреля 2026 — мок
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      <Glass s={s} glow>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
          <span style={{ fontFamily: H, fontSize: 22, color: s.text }}>Мой день</span>
          <span style={{ fontSize: 11, color: s.tS }}>22 апреля, вт</span>
        </div>
        <div style={{ display: "flex", gap: 6, marginTop: 10 }}>
      ...
      <SectionLabel s={s}>Статистика за {MOCK.stats.monthBlock.label}</SectionLabel>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
        <Glass s={s} style={{ padding: "12px 10px", textAlign: "center" }}>
          <div style={{ fontSize: 18, marginBottom: 4 }}>💰</div>
          <div style={{ fontFamily: H, fontSize: 18, color: s.acc, fontWeight: 500 }}>
            {MOCK.stats.monthBlock.inc.toLocaleString()}<span style={{ fontSize: 13, marginLeft: 1 }}>₽</span>
          </div>
        ...
        <Glass s={s} style={{ padding: "12px 10px", textAlign: "center" }}>
          ...
          <div style={{ fontFamily: H, fontSize: 18, color: s.text, fontWeight: 500 }}>
            {MOCK.stats.monthBlock.supplies.toLocaleString()}<span style={{ fontSize: 13, marginLeft: 1 }}>₽</span>
          </div>
        ...
        <Glass s={s} style={{ padding: "12px 10px", textAlign: "center" }}>
          <div style={{ fontSize: 18, marginBottom: 4 }}>✨</div>
          <div style={{ fontFamily: H, fontSize: 18, color: s.acc, fontWeight: 500 }}>
            {MOCK.stats.monthBlock.accuracy}%
          </div>
        ...
        <Glass s={s} style={{ padding: "12px 10px", textAlign: "center" }}>
          <div style={{ fontSize: 18, marginBottom: 4 }}>🃏</div>
          <div style={{ fontFamily: H, fontSize: 18, color: s.text, fontWeight: 500 }}>
            {MOCK.stats.monthBlock.sessions}
          </div>

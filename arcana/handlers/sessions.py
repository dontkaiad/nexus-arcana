...
PARSE_SESSION_SYSTEM = (
    ...
    'Род|Общая ситуация или null (см. правило ниже)", "spread_type": "Триплет", '
    ...
    "→ 'Общая ситуация' (это ЯВНЫЙ триггер, не дефолт-заглушка — ставь как есть)\n"
    ...
    "Слово/вопрос НЕ подходит НИ ПОД ОДИН из бакетов выше, ВКЛЮЧАЯ 'Общая "
    "ситуация' (расплывчатые формулировки вроде 'диагностика', 'что "
    "происходит', 'как дела у', без указания сферы) → area = null. НЕ "
    "натягивай такой вопрос на 'Общая ситуация' силой — это отдельный триггер "
    "для СВОИХ формулировок ('общее состояние' и т.п.), а не catch-all для "
    "всего непонятного. null здесь — честный сигнал неуверенности, код сам "
    "решит, чем его дополнить (историей темы или дефолтом).\n"
...
AREA_HINT_SYSTEM = (
    "Определи area (тема вопроса) по тексту. Ответь ТОЛЬКО JSON без markdown:\n"
    '{"area": "Отношения|Финансы|Работа|Здоровье|Род|Общая ситуация или null"}\n\n'
    "Сначала проверь явные ключевые слова в самом вопросе — они ПРИОРИТЕТНЕЕ "
    "любой подсказки:\n"
    "- 'общее состояние', 'общий обзор', 'как лег ритуал', 'что вообще' "
    "→ 'Общая ситуация'\n"
    "- 'что чувствует', 'любит ли', 'отношения', 'совместность', "
    "'сойдётся ли', 'тревожит в отношениях' → 'Отношения'\n"
    "- 'секс', 'тело', 'здоровье', 'болезнь', 'самочувствие' → 'Здоровье'\n"
    "- 'работа', 'карьера', 'устроится ли', 'увольнение', 'коллеги' → 'Работа'\n"
    "- 'деньги', 'финансы', 'доход', 'долги', 'покупка' → 'Финансы'\n"
    "- 'род', 'предки', 'родовое' → 'Род'\n\n"
    "Если явного триггера НЕТ (текст сам по себе неоднозначен — 'диагностика', "
    "'что происходит', 'как дела у' и т.п.): в запросе будет ПОДСКАЗКА — "
    "недавние area той же темы (тот же человек/субъект вопроса в прошлых "
    "сессиях). Используй её как best-guess: выбери ту область из подсказки, "
    "что ближе всего звучит к вопросу. Подсказка — НЕ жёсткое правило: если "
    "текст явно противоречит ей (например подсказка 'Отношения', а вопрос "
    "прямо про деньги) — доверяй тексту, а не подсказке.\n"
    "Если ни триггера в тексте, ни применимой подсказки нет — area = null."
)
...
async def _reclassify_area_with_subject_hint(question: str, subject_id: int) -> str:
    """area-классификатор при null от основного парсера (#190): контекстная
    подсказка из истории ТЕМЫ (subject_id), НЕ анкер — в отличие от
    _resolve_category (client anchor, ADR-0018/#174) сюда идём ТОЛЬКО когда
    у самого вопроса нет явного триггера, а не всегда. Явные слова в вопросе
    остаются приоритетнее (см. AREA_HINT_SYSTEM); эта функция лишь даёт
    Haiku лучший контекст, а не жёстко наследует прошлую area.

    Нет истории по теме → отдельного вызова Haiku не делаем (дефолт и так
    вернётся из _normalize_area("") у вызывающего кода) — деньги Кай не жжём
    на LLM-вызов, которому нечем подсказать.
    """
    recent = await _repo.recent_areas_for_subject(subject_id, limit=3)
    if not recent:
        return AREA_DEFAULT
    hint = ", ".join(recent)
    user_prompt = f"Вопрос: {question}\nПодсказка (недавние area этой темы): {hint}"
    try:
        raw = await ask_claude(user_prompt, system=AREA_HINT_SYSTEM, max_tokens=50, temperature=0)
    except Exception as e:
        logger.warning("area hint reclassify failed: %s", e)
        return AREA_DEFAULT
    data = _parse_json_safe(raw)
    if not data:
        return AREA_DEFAULT
    return _normalize_area(data.get("area") or "")
    ...
    *,
    ...
    for idx, it in enumerate(items, 1):
        try:
            ...
            raw_area = it.get("area")
            if not raw_area and known_subject_id:
                # Основной парсер вернул null (нет явного триггера в тексте
                # этого вопроса) — подсказка из истории темы, не жёсткое
                # наследование (#190). Дефолт "Общая ситуация" остаётся
                # доступен: если у темы тоже нет истории area, вернётся он.
                area = await _reclassify_area_with_subject_hint(question, known_subject_id)
            else:
                area = _normalize_area(raw_area or "")

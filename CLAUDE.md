...
**Issues** — под отслеживаемую/обсуждаемую работу: баги, фичи, нетривиальные и отложенные задачи.
Трекируемый баг: Кай описывает → `gh issue create` + лейбл + тип → номер #N → коммит с `fixes #N` → push → issue автозакрывается.
Трекируемая фича: идея → issue + acceptance criteria → `closes #N` при реализации.

**Тривиальный chore** (init инструмента, создание config/doc-файла, мелкая правка) — **БЕЗ issue**:
достаточно conventional-commit формата (`chore(scope): subject` / `docs(scope): subject` / `feat(scope): subject`).

**Семантика сообщения:**
- `fixes #N` / `closes #N` — только в коммите, который реально закрывает трекнутую issue.
- "Что" — в commit message (conventional format).
- "Почему" — в ADR и docs/CASES/.
...
## Статус (обновлено 12 июня 2026, после волны Arcana v8)
...
- **Тесты**: 1062 passed, 0 skipped, 0 failed. Полный sweep ~4:18.
  После чистки (#88): 71 файл pytest (было 85), 786 тест-функций
  (было 879), копипаста схлопнута через `@pytest.mark.parametrize`,
  E2E-раннеры вынесены в `tests/e2e/` (вне pytest-discovery).

## Стиль Кай (КРИТИЧНО)

СДВГ. Никаких стен текста. Никаких опросников когда уже всё сказано.
Краткие реплики, мат, EN/RU mix. Архитектуру обсуждаем ДО промпта,
не сразу промпт.

## Кто пишет промпты

Кай (она/её), AI/LLM-инженер. СДВГ. Все AI-промпты внутри ботов используют 
...
## ⌨️ Эргономика под СДВГ

Кай ревьюит каждое архитектурное решение и должна уметь его защитить.
Полные команды и минимальные чек-листы — чтобы СДВГ не терял нить на
механике (копипаст, переключения между окнами), а не потому что Кай
избегает понимания. Объяснять даже базовые шаги (как открыть папку
в терминале) — норма: она знает, но из-за СДВГ забывает.
...
Каждое значимое архитектурное решение документируется как ADR в `docs/CASES/`:
- что выбрали;
- какие альтернативы рассмотрели и почему отвергли;
- trade-offs;
- rationale.

ADR — чистый инженерный артефакт (портфолио-плюс, сигнал сеньорности).

ПРАВИЛО ЯЗЫКА: все доки и спеки (`docs/`, ADR, спеки) пишутся на английском.

ПРАВИЛО ПРИВАТНОСТИ: ADR в `docs/CASES/` содержат ТОЛЬКО инженерное обоснование. НИКОГДА — заготовки ответов для собеседований, заметки «здесь я плаваю», или любую мета о пробелах автора в знаниях. Это живёт вне репо (приватный Notion). Если контент касается личной подготовки Кай к собесу — он НЕ в репо.
...
- pytest, **1062 теста зелёных** (0 skipped)
...
├── tests/                       # 1062 теста pytest (e2e/ — отдельно)
...
- `CLAUDE_PROJECT_INSTRUCTIONS_v9.md` — инструкции проекта

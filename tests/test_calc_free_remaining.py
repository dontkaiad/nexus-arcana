...
подписью — числа расходились (60к в боте vs 6к в приложении), и ни одна не
вычитала реальные 📦 Разовые траты. Теперь обе стороны зовут ОДНУ функцию —
`core.budget.discretionary_free`:
...
формулу — лимиты, декремент тратами, вычет разовых, guard без лимитов.
"""
...
# ── (4) 📦 Разовые ФАКТИЧЕСКИ вычитаются — это и есть #237 фикс ────────────

@pytest.mark.asyncio
async def test_one_off_expense_is_subtracted():
    """Раньше 📦 Разовые не вычитались НИГДЕ (ни в боте, ни в приложении) —
    Кай замечала завышенный остаток относительно реальной карты."""
    limits = {"продукты": 10000}
    without_one_off, _ = await _call(limits, [])
    with_one_off, _ = await _call(limits, [_entry(4000, "📦 Разовые")])
    assert without_one_off[0] - with_one_off[0] == 4000.0

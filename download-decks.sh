#!/bin/bash
# Скачивание картинок колод для Mini App
# Запуск: ./download-decks.sh
# Кладёт в: $PROJECT/miniapp/frontend/public/decks/<deck>/<card>.jpg
...
fetch() {
  ...
  if [ "$code" = "200" ] && [ -s "$out" ]; then
    ...
    echo "  ❌ $(basename "$out") — HTTP $code"
    rm -f "$out"
  fi
}

echo ""
echo "═══ RIDER-WAITE (78 карт) ═══"
cd "$DECKS_DIR/rider-waite"

BASE="https://upload.wikimedia.org/wikipedia/commons"

# Формат: "output_filename|wikimedia_path"
CARDS=(
  # Major Arcana (0-21)
  ...
  # Wands (Жезлы) 1-14: ace, 2-10, page, knight, queen, king
...
echo "Rider-Waite: ${count}/78 карт в $DECKS_DIR/rider-waite/"
echo ""
echo "Ленорман и Атласные колоды — отдельный сбор (public domain, но на Wikimedia не в готовом виде)."
echo "Добавим когда сделаем основной функционал таро."

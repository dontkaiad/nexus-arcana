...
async def cmd_help(msg: Message, user_notion_id: str = "") -> None:
    await msg.answer(
        ...
        "Дедлайн и напоминание: «до пятницы», «напомни в 10».\n"
        "Повторяющиеся задачи: «каждую неделю», «ежедневно».\n"
        "В уведомлениях кнопки ✅ выполнено · ❌ отмена · ⏳ отложить.\n"
        "<code>/tasks</code> — список всех активных задач.\n\n"

        "💰 <b>ФИНАНСЫ</b>\n"
        "Расход: просто «450р такси» или «кофе 180».\n"
        "Доход: нужно слово «получила», «зарплата», «пришло» и т.д.\n"
        "Лимит по категории: «лимит на кафе 5000р».\n"
        "Предупреждения при 80% и 100% от лимита — автоматически.\n"
        "<code>/finance</code> — история · <code>/finance_stats</code> — сводка месяца с ревью лимитов.\n\n"

        "💡 <b>ЗАМЕТКИ</b>\n"
        "Создать: «заметка:», «идея:», «запомни рецепт:» — и дальше текст.\n"
        "Теги определяю сама; можно добавить свой: #тег прямо в тексте.\n"
        "<code>/notes</code> — последние заметки · <code>/notes_digest</code> — дайджест старых.\n\n"

        "🧠 <b>ПАМЯТЬ</b>\n"
        "Запоминаю факты о людях, котах, здоровье, предпочтениях, паттернах.\n"
        "Сохранить: «запомни что маша не ест мясо», «у меня аллергия на...».\n"
        "Найти: «напомни про машу», «что знаешь о батоне».\n"
        "Деактивация и удаление — прямо из результатов поиска.\n"
        "11 категорий: люди · коты · здоровье · привычки · предпочтения · работа · финансы · события · паттерны · контакты · прочее.\n\n"

        "🌍 <b>ЧАСОВОЙ ПОЯС</b>\n"
        "Установить: «я в москве», «utc+5» — и всё готово.\n"
        "Все напоминания и дедлайны работают по твоему времени.\n\n"

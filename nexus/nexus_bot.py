...
async def cmd_help(msg: Message, user_id: str = "") -> None:
    await msg.answer(
        ...
        "<b>Бюджет:</b>\n"
        "• «лимит привычки 15к», «закрыла долг ***»\n"
...
💅 Бьюти, 🏥 Здоровье, 💻 Подписки, 🏠 Жильё, 👗 Гардероб, 💳 Прочее
...
async def main() -> None:
    ...
    async def _on_startup(**kwargs) -> None:
        ...
        # NB: booking API живёт в этом же процессе — отдельный «Booking API up»
        # пинг убран (#23): он дублировал nexus-пинг на КАЖДОМ деплое и раздувал
        # уведы до трёх. Топик букинга (1182) остаётся для ошибок + деплоев Zarya.

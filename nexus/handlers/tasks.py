...
async def handle_tasks_today(message: Message, user_notion_id: str = "") -> None:
    ...
    try:
        # Передаём реальные названия задач — до 5 штук
        _tip_tasks = [it["title"] for it in (overdue + today_tasks + daily)[:5]]
        _tip_tasks_str = ", ".join(f"«{t}»" for t in _tip_tasks) if _tip_tasks else "нет"
        prompt = (

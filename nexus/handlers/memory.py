...
async def handle_memory_list(
    ...
    try:
        if matched_cat:
            mems = await _mem_repo.find_by_category(matched_cat, is_current=True, page_size=200)
        else:
            mems = await _mem_repo.find_recent(is_current=True, page_size=200)
            if exclude_adhd:
                mems = [m for m in mems if m.category != "🦋 СДВГ"]
            if exclude_budget:
                mems = [m for m in mems if m.category not in _BUDGET_CATS]
...
async def cb_mem_reactivate_all(call: CallbackQuery) -> None:
    ...
    if not pages:
        await call.message.edit_text("⏱ Сессия истекла.")
        return
    inactive_ids = [p.id for p in pages if not p.is_current]
...
async def cb_mem_delete_selected(call: CallbackQuery) -> None:
    ...
    for page in targets:
        try:
            await _mem_repo.archive(page.id)
            done += 1
        except Exception as e:

...
async def build_budget_message(user_notion_id: str = "") -> Optional[str]:
    ...
    # ── Хелпер: убрать "(🏠 Ж***)" из названия обязательного ──
...
BUDGET_SONNET_SYSTEM = (
    ...
    "  - Пример: *** 50к (апрель), *** 20к/мес → в марте платёж = ТОЛЬКО ***.\n"
    "    *** начнёт С АПРЕЛЯ (после закрытия ***)\n"
...
- "долг *** 50к до апреля" → name=***, amount=50000, deadline=апрель 2026
- "*** 100к август" → name=***, amount=100000, deadline=август 2026
- "должна *** 40к до сентября" → name=***, amount=40000, deadline=сентябрь 2026
"к" = тысяч. Если год не указан — текущий (2026) или следующий.
Текст: {user_text}
Верни ТОЛЬКО JSON массив (без markdown). Если долгов нет — верни [].
[{{"name": "***", "amount": 50000, "deadline": "апрель 2026"}}]"""
...
- Отложен: "после ***", "потом" → monthly_payment = 0
...
[{{"name": "***", "strategy": "25к апрель + 25к май", "monthly_payment": 25000}}]"""
...
async def start_budget_setup(message: Message, user_notion_id: str = "") -> None:
    ...
    sent = await message.answer(
        ...
        "долги: *** 50к до апреля, *** 40к до августа\n"
...
async def handle_budget_setup_text(message: Message, user_notion_id: str = "") -> bool:
    ...
    if len(debts_to_check) > 1:
        ...
        await message.answer(
            ...
            "<i>Например: «*** закрою в апреле, *** наследством, *** по 20к с мая»</i>".format(debts_text),
            parse_mode="HTML",
        )
        return True

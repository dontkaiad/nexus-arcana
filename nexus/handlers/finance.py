...
1. ФИКС — не трогать (ж***, коммуналка, подписки, интернет, вода, коты)
...
{{
  ...
  "debts_plan": [
    {{"name": "***", "amount": 50000, "monthly": 50000, "note": "разом в апреле"}}
  ],
  "goals_plan": [
...
async def on_budget_skip_variable(call: CallbackQuery) -> None:
    ...
    await call.message.edit_text(
        ...
        "<code>*** 50000 до апреля\n"
        "*** 40000 до августа</code>\n\n"
...
async def handle_budget_setup_text(message: Message, user_notion_id: str = "") -> bool:
    ...
    if state.step == "income":
        ...
        await _bot_edit_or_send(
            ...
            "Нельзя сократить: ж***, коммуналка, подписки, коты.\n"
    ...
    if state.step == "debts":
        lines = [l.strip() for l in text.split("\n") if l.strip()]
        for line in lines:
            # "*** 50к до апреля", "*** 40000 до августа", "*** 100к до сентября"
        ...
        if not state.saved_debts:
            await _bot_edit_or_send(
                ...
                "Например: <code>*** 50000 до апреля</code>",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
                    InlineKeyboardButton(text="Нет долгов → цели", callback_data="bsetup_skip_debts"),
                ]]),
            )
